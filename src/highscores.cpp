...
bool Highscores::AddScore(const std::string& name, uint32_t score, Difficulty difficulty) {
...
void Highscores::Priv_LoadHighscores() {
    try {
        ...
        for (std::string line; getline(is, line);) {
            ...
            Difficulty difficulty = static_cast<Difficulty>(std::stoi(line.substr(secondSplit + 1, line.size() - 1)));
            m_highscores.push_back(std::make_tuple(name, score, difficulty));
        }
...
void Highscores::Priv_SortHighscores() {
    std::sort(m_highscores.begin(), m_highscores.end(),
        [](std::tuple<std::string, uint32_t, Difficulty> a, std::tuple<std::string, uint32_t, Difficulty> b) {
